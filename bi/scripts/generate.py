...
def journey_for(customer, scenario):
    ...
    j = {
        ...
        "outcome": outcome,
        "days_to_placement": days_to_placement if placed_at else None,
        "placed_date": placed_at.isoformat() if placed_at else "",
...
def kpi_aggregates(journeys):
    ...
    for (region, adv, month), b in buckets.items():
        rows.append({
            ...
            "avg_days_to_placement": round(b["days_sum"]/b["days_n"],1) if b["days_n"] else None,
